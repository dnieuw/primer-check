...
ui <- dashboardPage(skin = "red",
        ...
        dashboardSidebar(
            ...
            br(),
            h5("The nCoV whole genomes were kindly shared via", a(href="https://www.gisaid.org/", target = "_blank", img(src="gisaid.png", align = "middle", width="50%"))),
            br()
        ), 
     dashboardBody(
         tabItems(
             tabItem(tabName = "input",
                     fluidPage(
                         ...
                         column(width = 4,
                                box(title = "Advanced", width = NULL, status = "warning", solidHeader = TRUE, collapsible = T, collapsed = T,
                                ...
                                box(width = NULL,
                                    ...
                                    actionButton("run_button", "LOAD/RUN"),
             ...
             tabItem(tabName = "source",
                 fluidPage(
                     box(width=12,
                         ...
                         rHandsontableOutput("source_table")
                     )
                 )
             )
         )
...
Target 2	ACGATTGTGCATCAGCTGA	RE	Institute X	Orf1b	2",header=T)
    ...
    output$query_select_heatmap <- renderPlotly({
        ...
        p <- ggplot(data, aes(y=paste(qname,type,sep='|'), x=gene, fill=mm, text=qname, key=qname)) + 
    ...
    output$source_table <- renderRHandsontable({
        hot <- rhandsontable(fread("gisaid_cov2020_acknowledgement_table.csv"), readOnly = TRUE, height = 800, allowedTags = "<a>") %>% 
...
#rsconnect::deployApp('.', account = "dnieuw", appName = "primer-check", appTitle = "nCoV primer check visualization")
